...
def matmul_tiled(
    ...
    for k in range(0, K, TILE_K):
        value_a_ptr  = a_ptr + OFFSET_BLOCK( offset_m[:,None], k + offset_k[None,:], 1,1, K)
        value_a_mask_k = (k + offset_k[None, :]) < K
        value_a      = tl.load(pointer=value_a_ptr,mask = value_a_mask & value_a_mask_k,other=0.0)     # [TILE_M,1] 
        
        value_b_ptr  = b_ptr + OFFSET_BLOCK(k + offset_k[:,None], offset_n[None,:], 1,1, N)
        value_b_mask_k = (k + offset_k[:, None]) < K
        value_b      = tl.load(pointer=value_b_ptr,mask = value_b_mask & value_b_mask_k,other=0.0)     # [1,TILE_N]
        
        psum_mat += tl.dot(value_a, value_b, allow_tf32=False)
